...
public class CoreEventHandler implements EventHandlerInt, InformationServiceProviderInt {
    ...
    public void invoke(OutputEvent e, ArrayList<Token> tokens) {
        LOGGER.log(Level.INFO, "CoreEventHandler invoked with " + e);
        if (e instanceof AbortMission) {
            for (Token token : tokens) {
                if (token.getProxy() != null) {
                    token.getProxy().abortMission(e.getMissionId());
                }
            }
        } else if (e instanceof SendProxyAbortAllMissions) {
            for (Token token : tokens) {
                if (token.getProxy() != null) {
        ...
        } else if (e instanceof SendProxyAbortFutureMissions) {
            for (Token token : tokens) {
                if (token.getProxy() != null) {
                    // Add proxy that will be aborting the missions
                    ArrayList<ProxyInt> relevantProxies = new ArrayList<ProxyInt>();
        ...
        } else if (e instanceof SendAbortMission) {
            ...
            AbortMissionReceived pamr = new AbortMissionReceived(e.getMissionId());
            for (GeneratedEventListenerInt listener : listeners) {
                listener.eventGenerated(pamr);
            }
        }
